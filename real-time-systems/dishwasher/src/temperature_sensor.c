...
#define TIME_BETWEEN_READS 2000

/* 
* ############################################ 
*
* Sensor that watches the temperature of the pumped water.
* This data is written on an alarm cycle of 2 seconds to the temperature data module
*
* ############################################
*/
sig_handler(signal_code sig)
{
    switch(sig)
    {
		case 400: 
			_os_exit(0);
        case 301:
            printf("TEMPERATURE_SENSOR: Retrieved signal to read from I\O \n");
            break;
    }
	_os_rte();
}
...
{									        
    ...
    */
    u_int32 index = 0;
    u_int32 dummy_sensor[] = {10, 100, 30, 100, 50, 60, 70, 100, 100, 100};
    ...
    */
    ...
    printf("TEMPERATURE_SENSOR: Process started \n");

	if((err = _os_intercept(sig_handler, _glob_data)) != 0)
    {
        printf("TEMPERATURE_SENSOR: Error setting up signal handler \n");
        _os_exit(err);
    }  
    ...
    {
        printf("TEMPERATURE_SENSOR: Couldn't link to ProcessesList data module \n");
        _os_exit(err);
    }

    if((err = _os_alarm_cycle(&sensor_alarm, sensor_rdy_signal, time_between_reads)) != 0)
    {
        printf("TEMPERATURE_SENSOR: Error creating alarm cycle \n");
        _os_exit(err);
    }
	
	printf("TEMPERATURE_SENSOR: Ready \n");
    while(1)
    {
        _os_sleep(&time, &received_signal);
        if(received_signal == SENSOR_RDY_SIGNAL)
        {
            if(index == SENSOR_ARR_SZ)
            {
                index = 0;
            }
             write_temperature(dummy_sensor[index]);
			_os_send(proc_list->p_ids[WATER_HEATER_INDEX], 302);
            _os_send(proc_list->p_ids[DETERGENT_DISPENSER_INDEX], 302);
            index++;
        }    
    }
}