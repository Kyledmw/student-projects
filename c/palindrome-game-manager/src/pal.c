...
/*
* Get the length in terms of the amount of digits of the given number
*/
...
void machine_game_palindrome(int pal_num, int size_num, char commands[], int command_size) {
	...
	char* instructions = ' ';
	instructions = get_solving_array(a, size_num, p, pal_num, &optimal_instruction_count);
...
/*
 * Calculates the optimal amount of movements requires, 
 * Stores this in the value of the given pointer optimal_instruction_count_ptr
 * It then gets an array of the instructions required
 */
char* get_solving_array(int* a, int size_num, int* p, int pal_num, int* optimal_instruction_count_ptr) {
	int* initial_p = p;
	int use_latter_half_of_a = 0;
	...
	*optimal_instruction_count_ptr = optimal_instruction_count;
